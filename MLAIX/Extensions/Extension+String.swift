...
public extension String {
    ...
    func splitByLatex() -> [(string: String, isLatex: Bool)] {
        ...
        guard let regex = try? NSRegularExpression(
            ...
            return [(string: self, isLatex: false)]
        }
    ...
    var reasoningProcess: String? {
        ...
        for specialTokenSet in specialTokenSets {
            guard specialTokenSet.count >= 2 else { continue }
            let startToken = specialTokenSet[0]
            let endToken = specialTokenSet[1]
            // Get range of start token
            if let startRange = self.range(of: startToken) {
    ...
    var reasoningRemoved: String {
        ...
        for tokenSet in specialTokenSets {
            guard tokenSet.count >= 2 else { continue }
            let startToken = tokenSet[0]
            let endToken = tokenSet[1]
            // If only the first token is found, return empty response
            if self.contains(startToken) && !self.contains(endToken) {
                return ""
            }
            // Extract text between tokens
            if let startRange = processedResponse.range(of: startToken),
    ...
    func extractEmailAddresses() -> [String] {
        ...
        do {
            ...
            Logger(subsystem: Bundle.main.logSubsystem, category: "String").error("\(error.localizedDescription, privacy: .public)")
        }
        return results
    }
    ...
    /// Parses a chart cell value (handles commas, spaces, percent suffix).
    /// Returns nil if the string cannot be parsed as a number.
    func parseChartNumericValue() -> Double? {
    ...
    private static let referenceSuffixes = [
    ...
    /// Strips common reference/source section suffixes from message text.
    func stripReferenceSuffixes() -> String {
        var result = self
        for suffix in Self.referenceSuffixes {
            result = result.dropSuffixIfPresent(suffix)
        }
        return result
    }
