...
struct PromptInputField: View {
    ...
    @State private var delayedClearTask: Task<Void, Never>?
    ...
    var body: some View {
        textField
            ...
            .onChange(of: conversationState.selectedExpertId) { _, _ in
    ...
    private func setupKeyEventMonitor() {
        ...
        if self.keyEventMonitor == nil {
            self.keyEventMonitor = NSEvent.addLocalMonitorForEvents(
                ...
                guard self.shouldHandleMonitoredKeyEvent(event) else {
                    return event
                }
    ...
    private func handleKeyDownEvent(_ event: NSEvent) -> Bool {
        ...
        switch returnKeyAction(for: event) {
            case .submit:
                self.onSubmit()
                return true
            case .insertNewline:
                self.insertNewlineAtInsertionPoint()
                return true
            case .passThrough:
                return false
        }
    ...
    private func shouldHandleMonitoredKeyEvent(_ event: NSEvent) -> Bool {
    ...
    private func returnKeyAction(for event: NSEvent) -> ReturnKeyAction {
        ...
        if !self.useCommandReturn && noModifiers {
            return .submit
        }
        if isShiftKeyDown || isOptionKeyDown || (self.useCommandReturn && noModifiers) {
            return .insertNewline
        }
        return .passThrough
    }

    private func insertNewlineAtInsertionPoint() {
        Task { @MainActor in
            withAnimation {
                let safeInsertionPoint = self.clampedInsertionPoint(
                    self.promptController.insertionPoint,
                    in: self.promptController.prompt
                )
                let index: String.Index = self.promptController.prompt.index(
                    self.promptController.prompt.startIndex,
                    offsetBy: safeInsertionPoint,
                    limitedBy: self.promptController.prompt.endIndex
                ) ?? self.promptController.prompt.endIndex
                self.promptController.prompt.insert(
                    "\n",
                    at: index
                )
                self.promptController.insertionPoint = safeInsertionPoint + 1
            }
        }
    }

    private func clampedInsertionPoint(_ insertionPoint: Int, in text: String) -> Int {
        max(0, min(insertionPoint, text.utf16.count))
    }
    ...
    private func startImageGeneration() {
        promptController.imageConcept = promptController.prompt
        promptController.isGeneratingImage = true
        promptController.sentConversation = nil
    }
    ...
    private func generateChatResponse(
        ...
        do {
            ...
            Self.logger.error("Agent listen threw unexpected error: \(String(describing: error), privacy: .public)")
            return
        }
        // Update UI
        await MainActor.run {
...
""",
    ...
    private func scheduleDelayedClear() {
        ...
        let task = Task { @MainActor in
            ...
            delayedClearTask = nil
        }
        delayedClearTask = task
    }
    
    private func cancelDelayedClear() {
        delayedClearTask?.cancel()
        delayedClearTask = nil
    }
