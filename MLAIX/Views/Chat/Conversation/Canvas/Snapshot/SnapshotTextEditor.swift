...
struct SnapshotTextEditor: View {
	...
	var body: some View {
		...
		.onChange(of: self.conversationState.selectedExpertId) { _, _ in
			self.loadSnapshotText()
		}
		.onChange(of: self.canvasController.selectedMessageId) { _, _ in
			self.loadSnapshotText()
		}
		.onChange(of: self.selectedSnapshotId) { _, _ in
			self.loadSnapshotText()
		}
		.onChange(of: self.text, initial: false) { _, _ in
