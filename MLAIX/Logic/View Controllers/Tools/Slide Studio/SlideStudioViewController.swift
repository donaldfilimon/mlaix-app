...
"""
	...
	private func generateMarkdown(
		...
		let cheatsheetText: String = (try? String(contentsOf: cheatsheetUrl, encoding: .utf8)) ?? ""
		// Formulate prompt
		do {
...
"""
	...
	public func startPreview() {
		...
		do {
			...
			// Print error
			Logger(subsystem: Bundle.main.logSubsystem, category: "SlideStudioViewController").error("Error generating diagram: \(error.localizedDescription, privacy: .public)")
	...
	public func exportSlides(
		...
		do {
			...
			// Print error
			Logger(subsystem: Bundle.main.logSubsystem, category: "SlideStudioViewController").error("Error generating slides: \(error.localizedDescription, privacy: .public)")
			// Return to first step
			Task { @MainActor [weak self] in
				Dialogs.showAlert(
					title: String(localized: "Error"),
