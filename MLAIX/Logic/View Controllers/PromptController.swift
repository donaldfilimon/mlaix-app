...
public class PromptController: ObservableObject, DropDelegate {
    ...
    private func createRecognitionRequest() {
        ...
        guard recognitionRequest != nil else {
            Self.logger.error("Unable to create speech recognition request")
            return
        }
            ...
            Task { @MainActor [weak self] in
                switch authStatus {
                    ...
                    case .denied, .restricted, .notDetermined:
                        ...
                        self?.stopAudioEngine()
                    @unknown default:
                        Self.logger.warning("Unknown speech authorization status: \(authStatus.rawValue, privacy: .public)")
                        self?.stopAudioEngine()
                }
            }
        }
