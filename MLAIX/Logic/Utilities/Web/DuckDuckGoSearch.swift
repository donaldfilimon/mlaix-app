...
public class DuckDuckGoSearch {
    ...
    private static func decodeHTMLEntities(
        ...
        guard let decimalRegex = try? NSRegularExpression(pattern: decimalPattern, options: []) else {
            return result
        }
        ...
        guard let hexRegex = try? NSRegularExpression(pattern: hexPattern, options: []) else {
            return result
        }
