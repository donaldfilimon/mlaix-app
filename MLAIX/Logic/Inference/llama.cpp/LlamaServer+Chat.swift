...
extension LlamaServer {
                        ...
                        do {
                            ...
                            if responseObj.choices.first?.finish_reason != nil {
                                do {
                                    ...
                                    Self.logger.warning("Error decoding stopResponse, listenLoop will continue: \(String(describing: error), privacy: .public) \(data.count) bytes")
                                }
                                break listenLoop
                            }
                        } catch {
    ...
    public func getCompletion(
        ...
        // Formulate request
        let urlString = "\(self.scheme)://\(self.host):\(self.port)/v1/completions"
        guard let url = URL(string: urlString) else {
            Self.logger.error("Invalid completions URL: \(urlString, privacy: .public)")
            return nil
        }
        var request = URLRequest(
            url: url
        ...
        guard let requestJson = String(data: data, encoding: .utf8) else {
            Self.logger.error("Failed to encode completion params as UTF-8")
            return nil
        }
        request.httpBody = requestJson.data(using: .utf8)
        // Formulate session
