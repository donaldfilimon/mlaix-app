...
public class SourcesManager {
	...
	public func load() {
		do {
			...
			// Indicate error
			Logger(subsystem: Bundle.main.logSubsystem, category: "SourcesManager").error("Failed to load sources: \(error.localizedDescription, privacy: .public)")
			// Make new datastore
			self.newDatastore()
		}
	}
	...
	public func patchFileIntegrity() {
		...
		if !self.datastoreDirExists {
			do {
				...
				Logger(subsystem: Bundle.main.logSubsystem, category: "SourcesManager").error("Failed to create datastore directory: \(error.localizedDescription, privacy: .public)")
			}
		}
	}
