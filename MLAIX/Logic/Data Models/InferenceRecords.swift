    ...
    public var intervalUsage: [IntervalUse] {
        ...
        let grouping: (Date) -> Date = { date in
            switch timeframe {
                case .today:
                    // Group by hour.
                    return (calendar.dateInterval(of: .hour, for: date) ?? DateInterval(start: date, duration: 3600)).start
    ...
    public func patchFileIntegrity() {
        ...
        if !self.datastoreDirExists {
            do {
                ...
                Logger(subsystem: Bundle.main.logSubsystem, category: "InferenceRecords").error("Failed to create datastore directory: \(error.localizedDescription, privacy: .public)")
            }
        }
    }
