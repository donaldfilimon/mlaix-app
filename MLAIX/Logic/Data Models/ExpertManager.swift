...
public class ExpertManager {
    ...
    public func load() {
        do {
            ...
            Self.logger.error("Failed to load experts: \(error.localizedDescription, privacy: .public)")
            self.newDatastore()
        }
    }
    ...
    public func removeUnpersistedResources() {
        for index in self.experts.indices {
            if !self.experts[index].persistResources {
                ...
                Self.logger.info("Removed resources for expert \(self.experts[index].name, privacy: .public)")
            }
        }
    }
