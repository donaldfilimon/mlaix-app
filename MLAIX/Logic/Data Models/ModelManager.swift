...
public class ModelManager {
	...
	public func load() {
		do {
			...
			// Indicate error
			Logger(subsystem: Bundle.main.logSubsystem, category: "ModelManager").error("Failed to load models: \(error.localizedDescription, privacy: .public)")
			// Make new datastore
			self.newDatastore()
		}
	}
	...
	public func patchFileIntegrity() {
		...
		if !self.datastoreDirExists {
			do {
				...
				Logger(subsystem: Bundle.main.logSubsystem, category: "ModelManager").error("Failed to create datastore directory: \(error.localizedDescription, privacy: .public)")
			}
		}
	}
