...
"""
        ...
        for _ in 0..<5 {
            do {
                ...
                // Decode response
                guard let data = responseText.data(using: .utf8) else { continue }
                let decoder: JSONDecoder = JSONDecoder()
                let sections: [Section] = try decoder.decode([Section].self, from: data)
                return sections
            } catch {
                // Try again
...
"""
        ...
        for _ in 0..<3 {
            do {
                ...
                guard let data = responseText.data(using: .utf8) else { continue }
                let decoder: JSONDecoder = JSONDecoder()
                let results: [Section.Result] = try decoder.decode([Section.Result].self, from: data)
                // Check if empty
                if results.isEmpty {
...
"""
        ...
        for _ in 0..<4 {
            do {
                ...
                guard let data = responseText.data(using: .utf8) else { continue }
                let decoder: JSONDecoder = JSONDecoder()
                diagrams = try decoder.decode([Diagram].self, from: data)
                break
            } catch {
                // Try again
