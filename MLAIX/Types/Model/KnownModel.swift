...
public struct KnownModel: Identifiable, Codable, Sendable {
	private static let logger = Logger(subsystem: Bundle.main.logSubsystem, category: String(describing: KnownModel.self))
    ...
    public static func findModel(byIdentifier identifier: String) async -> KnownModel? {
        do {
            ...
            Self.logger.error("Failed to fetch models: \(error.localizedDescription, privacy: .public)")
            return nil
        }
    }
    ...
    public static func loadModelsFromFile() -> [KnownModel]? {
        do {
            ...
            Self.logger.error("Failed to load models from cache file: \(error.localizedDescription, privacy: .public)")
            return nil
        }
    }
    ...
    private static func saveModelsToFile(_ models: [KnownModel]) {
        do {
            ...
            Self.logger.error("Failed to save models to cache file: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    public static func getAvailableModelsSafe(includeOnlyKnownOrganizations: Bool = false) async -> [KnownModel] {
        do {
            ...
            Self.logger.error("Failed to fetch models from OpenRouter API: \(error.localizedDescription, privacy: .public)")
            return []
        }
    }
