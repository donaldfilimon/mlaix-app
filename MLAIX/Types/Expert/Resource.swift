...
public struct Resource: Identifiable, Codable, Hashable, Sendable {
    ...
    public func deleteDirectory(resourcesDirUrl url: URL) {
        ...
        do {
            ...
            Self.logger.error("Failed to remove resource directory at \"\(self.url.path, privacy: .public)\": \(error.localizedDescription, privacy: .public)")
        }
        // Indicate change
        Self.logger.info("Removed item at \"\(self.url.path, privacy: .public)\" from index")
    }
