...
public struct Message: Identifiable, Codable, Hashable, Sendable {
	...
	public var reasoningText: String? {
		...
		for specialTokenSet in specialTokenSets {
			...
			if let startRange = self.text.range(of: startToken) {
	...
	public func textWithSources(
        ...
        guard let jsonData = try? jsonEncoder.encode(sourcesInfo),
              let resultsText = String(data: jsonData, encoding: .utf8) else {
            return (self.text, 0)
        }
				...
				AsyncImage(
					...
					content: { image in
						let base = image
							...
							.contextMenu {
								Button { NSWorkspace.shared.open(url) } label: { Text("Open") }
							}
						if let nsImage = NSImage(contentsOf: url) {
							base.draggable(Image(nsImage: nsImage))
						} else {
							base
						}
					},
