...
public final class WebFunctions: Sendable {
    ...
    enum WebSearchError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
                case .encodingFailed:
                    return "Failed to encode search results"
                case .invalidDateFormat:
    ...
    static func tavilyWebSearch(
        ...
        return Function<TavilyWebSearchParams, String>(
            ...
            guard let jsonData = try? jsonEncoder.encode(sourcesInfo),
                ...
                throw WebSearchError.encodingFailed
            }
            return """
Below are the sites and corresponding content returned from your `web_search` query.

The content from each site here is an incomplete excerpt. Use the `get_website_content` function to get the full content from a website.
    ...
    static let standardWebSearch = Function<StandardSearchParams, String>(
        ...
        run: { params in
            ...
            switch provider {
                ...
                case .tavily:
                    sources = try await Tavily.search(
                        query: params.query,
                        site: params.site,
                        resultCount: numResults
                    )
