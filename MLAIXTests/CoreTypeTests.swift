...
struct SenderTests {
    ...
    func testSenderDecodeFromRawString(rawValue: String, expected: Sender) throws {
        let data = "\"\(rawValue)\"".data(using: .utf8)!
        let decoded = try JSONDecoder().decode(Sender.self, from: data)
        #expect(decoded == expected, "Decoding '\(rawValue)' should produce \(expected)")
    }
