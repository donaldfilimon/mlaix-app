...
./scripts/run-ui-tests.sh       # Convenience script for UI tests
```
...
- Executable targets: `MLAIX` (macOS), `MLAIXiOS` (iOS/iPadOS), `MLAIXtvos` (tvOS placeholder)
- Secondary target: `llama-server-watchdog` (process monitor)
- Test targets: `MLAIXTests` (path: `MLAIXTests/`), `MLAIXiOSTests`
...
Most managers are `@MainActor` classes with `static let shared` singletons, injected as `@StateObject`/`@EnvironmentObject` from `MLAIXApp.swift`:
...
- **`FoundationModelsSupport`** — Apple Intelligence on macOS 26+ (FoundationModels framework). Image Playground uses macOS 15.2+.
...
- **Multi-platform**: macOS (full), iOS/iPadOS (remote API only), tvOS (placeholder). Unified code in `MLAIXShared`. See `docs/PLATFORMS.md`.
